<h1 align="center">Hi 👋, I'm Avinash </h1>
<h3 align="center">Aspiring Data Analyst | SQL • Python • Tableau • Excel | Learning by Doing</h3>

---

🌟 **About Me**

- 👨‍💻 I’m transitioning into the world of **Data Analytics** after 1+ year in the IT-BPO industry (Hexaware, Amazon Process)  
- 📊 Passionate about **making sense of data** using tools like SQL, Python, Tableau & Excel  
- 🧠 Constantly learning through real projects and challenges at **AlmaBetter**  
- 🏆 Awarded **"Rising Star"** at Hexaware for outstanding performance  
- 📚 Currently sharpening my ML & Statistics knowledge  

---

🛠️ **Tech Stack I Use**

- **Languages:** Python (Pandas, NumPy), SQL  
- **Data Viz:** Tableau, Excel, Matplotlib, Seaborn  
- **Tools:** Jupyter, VS Code, Git  
- **Database:** MySQL  

---

📂 **Featured Projects**

| Project | Description | Tech |
|--------|-------------|------|
| 📊 Sales Dashboard | Interactive dashboard showing sales KPIs & trends | Tableau |
| 🧹 Data Cleaning | Cleaned messy datasets (nulls, outliers) into ready-to-analyze format | Python, Pandas |
| 📈 IPL Data Analysis | EDA on IPL dataset to find winning strategies | Python, Matplotlib |
| 🧠 ML Basics | Built a regression model to predict housing prices | Scikit-learn |

> 🔗 View all projects here: [My Projects Repo](#)  
> 🔗 Certification Portfolio: [View on Drive](#)

---

📫 **Let's Connect**

- [LinkedIn](https://www.linkedin.com/in/avinashjen)  
- [kaggle](https://www.kaggle.com/avinashjen)



<!--
**Avinashjen/Avinashjen** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
